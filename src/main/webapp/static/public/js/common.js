...
(function($) {
	...
	$.dialog = function(options) {
		...
		if (settings.ok != null) {
			$dialogOk = $('<input type="button" class="button" value="确&nbsp;&nbsp;定" \/>').appendTo($dialogBottom);
		}
		if (settings.cancel != null) {
			$dialogCancel = $('<input type="button" class="button" value="取&nbsp;&nbsp;消" \/>').appendTo($dialogBottom);
		}
		if (!window.XMLHttpRequest) {
	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				title: "选择文件",
				isUpload: true,
				browserUrl: e3dmall.base + "/businessman/file/browser.jhtml",
				uploadUrl: e3dmall.base + "/businessman/file/upload.jhtml",
				callback: null
			};
			$.extend(settings, options);
			...
			return this.each(function() {
				...
				$browserButton.click(function() {
					...
					if (settings.isUpload) {
						...
						$browserUploadButton = $('<a href="javascript:;" class="browserUploadButton button">' +"本地上传" + '<\/a>').appendTo($browserForm);
						$browserUploadInput = $('<input type="file" name="file" \/>').appendTo($browserUploadButton);
					}
					$browserParentButton = $('<a href="javascript:;" class="button">' + "上级目录" + '<\/a>').appendTo($browserBar);
					$browserBar.append("排序方式" + ": ");
					$browserOrderType = $('<select name="orderType" class="browserOrderType"><option value="name">' + "名称" + '<\/option><option value="size">' + "大小" + '<\/option><option value="type">' + "类型" + '<\/option><\/select>').appendTo($browserBar);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = e3dmall.base + "/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = e3dmall.base + "/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
